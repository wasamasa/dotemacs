...
flexibility (complex forms are unsupported).  For experimentation,
reload such configuration with =xrdb ~/.Xresources=.
...
separate file, to accomodate for that we need to customize a variable
...
the minimum prime bits size alleviates that.
...
from the =xdg-open= script).  Could be worth handing in a bug, I
guess.
...
*** Display .nfo files with appropriate codepage
...
Ideally it should prune the list the same way shells do when disabling
duplicates, something about it feels off.  I should investigate in it.
...
This is a great candidate for a patch that enables a more convenient
option, perhaps even a mailing list discussion about it.
...
I've got a bunch of other things to do than figuring out what other
REPL modes to activate it for, like making the UI behave like
~auto-complete-mode~ (should be done by deactivating common prefix
completion and overloading TAB), porting existing backends to make use
of =company-capf= and writing a completion style that feels like
=YouCompleteMe=, ideally with a binary component to make it fast.
...
Install a keybind that saves all buffers, then kills Emacs (including
the daemon) on =M-<f4>=.
...
happens /after/ successful startup.  Now that it's not 2007 anymore
and =package.el= has become a defacto standard, this might be a
legitimate topic to discuss on the =emacs-devel= mailing list.
