...
Promises to go beyond =paredit= by supporting other languages than
...
Finally, working on college assignments in both C and Java made me
wish for an interesting feature I've seen in IDEs: Automatic insertion
of a correctly indented newline before the closing brace which allows
you to enter its content right away.
...
for some reason ~org-capture-mode~ ignores an initial state, that's
why I use a mode hook instead.

#+BEGIN_SRC emacs-lisp
(add-hook 'org-capture-mode-hook 'evil-insert-state)
#+END_SRC

Go for a bit more Emacs-like feel for movement, undo, search.
...
~evil-normalize-keymaps~, at least for ~edebug-mode~.  For yet unknown
reasons it doesn't work for ~macrostep-mode~.
...
I'm not sure what to think of the =ace-jump=.  For convenience I've
...
(with-eval-after-load 'evil-maps
  ...
  (define-key evil-normal-state-map (kbd "ze") 'toggle-debug-on-error)
  (define-key evil-normal-state-map (kbd "zq") 'toggle-debug-on-quit)
  (define-key evil-normal-state-map (kbd "zt") 'my-solarized-toggle)
  (define-key evil-normal-state-map (kbd "zm") 'my-toggle-mode-line-minor-modes)
...
=rcirc= is too small, ERC is too large.  So I chose Circe as my IRC
...
useful like setting a X urgency hint, asides from that I want a bit
more of control to treat highlights in private queries different from
channel highlights.  The following code yanks out the default one and
replaces it with something slightly better.
...
honest, to discern 4chan people from the rest).
