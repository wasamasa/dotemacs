...
(check the value of ~evil-want-C-i-jump~), but for whatever reason, it
doesn't work properly.  To avoid errors about unbound keys, =C-i= is
otherwise bound to a keyboard macro that triggers whatever =TAB= would
do.
...
(with-eval-after-load 'evil-maps
  (define-key evil-normal-state-map "Y" 'my-evil-yank-to-eol))
#+END_SRC
...
(with-eval-after-load 'evil-maps
  (define-key evil-normal-state-map "zs" 'describe-char)
  (define-key evil-normal-state-map "ze" 'toggle-debug-on-error)
  (define-key evil-normal-state-map "zq" 'toggle-debug-on-quit)
  (define-key evil-normal-state-map "zt" 'my-solarized-toggle)
  (define-key evil-normal-state-map "zm" 'my-toggle-mode-line-minor-modes)
