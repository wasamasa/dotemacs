      ...
      (sxml-to-xml
       `(svg (@ (xmlns "http://www.w3.org/2000/svg")
             ...
             ,@(cl-loop for ((i . j) . value) in svg-2048-board collect
                        (let ((digits (if value (ceiling (log10 value)) 0))
                              (tile-roundness (/ svg-2048-roundness 2))
                              (x-tile (+ (* (1+ j) svg-2048-padding)
                                         (* j svg-2048-box-size)
                                         svg-2048-offset))
                              (y-tile (+ (* (1+ i) svg-2048-padding)
                                         (* i svg-2048-box-size)
                                         svg-2048-offset))
                              (x-text (+ (* (1+ j) svg-2048-padding)
                                         (/ (* (1- (* (1+ j) 2))
                                               svg-2048-box-size) 2)
                                         svg-2048-offset))
                              (y-text (+ (* (+ i 2) svg-2048-padding)
                                         (/ (* (1- (* (1+ i) 2))
                                               svg-2048-box-size) 2)
                                         svg-2048-offset)))
