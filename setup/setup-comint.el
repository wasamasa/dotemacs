...
(defun my-comint-goto-last-output-beg ()
  (comint-goto-process-mark)
  (while (not (eq (get-char-property (point) 'field) 'boundary))
    (goto-char (previous-char-property-change (point) (point-min))))
  (goto-char (1+ (point))))

(defun my-comint-goto-last-output-end ()
  (comint-goto-process-mark)
  (while (not (eq (get-char-property (point) 'font-lock-face)
...
(defun my-comint-clear-last-output ()
  (interactive)
  (let ((start (save-excursion (my-comint-goto-last-output-beg)))
        (end (save-excursion (my-comint-goto-last-output-end))))
