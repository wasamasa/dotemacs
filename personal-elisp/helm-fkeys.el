...
(setq helm-f1-data
      '(("a" . ("[A]propos" "helm-apropos"))
        ("b" . ("[B]indings" "describe-bindings"))
        ("c" . ("[C]ommand" "describe-key-briefly"))
        ("d" . ("[D]ocumentation" "apropos-documentation"))
        ("f" . ("[F]unction" "describe-function"))
        ("i" . ("[I]nfo" "info"))
        ("k" . ("[K]ey" "describe-key"))
        ("l" . ("[L]ossage" "view-lossage"))
        ("m" . ("[M]odes" "describe-mode"))
        ("p" . ("[P]ackages by category" "finder-by-keyword"))
        ("v" . ("[V]ariable""describe-variable"))
        ("w" . ("[W]hich key" "where-is"))))
